...
function estimateWavBytes(duration, sampleRate) {
  return Math.ceil(Math.max(0, duration) * Math.max(1, sampleRate)) * 2 * 4 + 114;
}
...
function App() {
  ...
  const [analyzing, setAnalyzing] = useState(false); const [exporting, setExporting] = useState(false); const [progress, setProgress] = useState(0); const [renderInfo, setRenderInfo] = useState(null); const [renderElapsed, setRenderElapsed] = useState(0); const [message, setMessage] = useState(''); const [showShortcuts, setShowShortcuts] = useState(false);
  ...
  const [audioBackend, setAudioBackend] = useState(() => api ? localStorage.getItem('r9club-audio-backend') || 'shared' : 'shared'); const [audioCapabilities, setAudioCapabilities] = useState(null); const [probingAudio, setProbingAudio] = useState(false); const [asioDriver, setAsioDriver] = useState(() => localStorage.getItem('r9club-asio-driver') || '');
  ...
  const estimatedRenderBytes = useMemo(() => estimateWavBytes(totalDuration, masterSampleRate), [totalDuration, masterSampleRate]);
  ...
  const exportMix = async (skipQuality = false) => {
    ...
    setRenderInfo({ phase: 'selecting', format: 'wav', quality: '32-bit Float PCM', sampleRate: masterSampleRate, duration: totalDuration, estimatedBytes: estimatedRenderBytes, filePath: '' });
    if (!api) {
      ...
      webRenderTimerRef.current = setInterval(() => {
        ...
        if (value >= 100) {
          ...
          setTimeout(() => setRenderInfo((current) => ({ ...current, phase: 'complete', verification: { passed: true, codec: 'pcm_f32le', sampleRate: masterSampleRate, channels: 2, duration: totalDuration, sampleCount: Math.round(totalDuration * masterSampleRate), peakDb: -0.84, fileBytes: estimatedRenderBytes, checks: { codec: true, sampleRate: true, channels: true, duration: true } } })), 850);
        }
      }, 80);
      return;
    }
    try {
      const result = await api.exportMix({ tracks });
      ...
      setMessage(result.verification?.passed ? `Render และตรวจ Lossless Master ผ่านแล้ว: ${result.filePath}` : `Render เสร็จ แต่ผลตรวจไฟล์มีข้อผิดพลาด: ${result.filePath}`);
  ...
  useEffect(() => {
    const onKeyDown = (event) => {
      ...
      if ((showShortcuts || qualityReport || showAudioPanel) && event.key === 'Escape') { event.preventDefault(); setShowShortcuts(false); setQualityReport(null); setShowAudioPanel(false); return; }
      if (showShortcuts || qualityReport || showAudioPanel) return;
      ...
      if (event.ctrlKey && key === 'e') { event.preventDefault(); exportMix(); return; }
      if (editable) return;
  ...
  }, [analyzing, auditionBars, bpm, historyStatus, playhead, playing, qualityReport, recoveryProject, selected, showShortcuts, showAudioPanel, tracks, zoom]);
  ...
  return <div className="app-shell">
    ...
    {showAudioPanel && <div className="modal-backdrop audio-backdrop" onMouseDown={() => setShowAudioPanel(false)}><section className="audio-dialog" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="audio-dialog-body">
        ...
        <div className="engine-diagnostics">
          <div><small>BACKEND</small><b>{audioBackend === 'asio' ? 'NAUDIO / ASIO' : audioBackend === 'wasapi-exclusive' ? 'WASAPI EXCLUSIVE' : 'CHROMIUM / WASAPI SHARED'}</b><span>{audioStats.state.toUpperCase()}</span></div>
    ...
    {exporting && <div className="modal-backdrop render-backdrop"><section className={`render-page ${renderComplete ? 'render-complete' : ''} ${renderFailed ? 'render-failed' : ''}`}>
      ...
      <div className="render-body"><div className="render-primary"><div className="render-status"><span>{renderPhase}</span><b>{progress}%</b></div><div className="render-progress"><i style={{ width: `${progress}%` }} /></div><div className="render-stages"><span className="active"><Check size={12} /> PREPARE</span><span className={progress > 0 ? 'active' : ''}><AudioLines size={12} /> MIX</span><span className={progress > 80 ? 'active' : ''}><Download size={12} /> ENCODE</span><span className={progress >= 99 ? 'active' : ''}><Check size={12} /> FINALIZE</span><span className={['verifying', 'complete', 'verification-failed'].includes(renderInfo?.phase) ? 'active' : ''}><ShieldCheck size={12} /> VERIFY</span></div></div>
        <div className="render-metrics"><div><small>FORMAT</small><b>WAV</b><span>32-bit Float PCM</span></div><div><small>SAMPLE RATE</small><b>{((renderInfo?.sampleRate || masterSampleRate) / 1000).toFixed(1)} kHz</b><span>Highest source rate</span></div><div><small>ARRANGEMENT</small><b>{tracks.length} TRACKS</b><span>{formatTime(totalDuration)} · No Fade</span></div><div><small>FILE SIZE</small><b>{formatBytes(renderInfo?.verification?.fileBytes || renderInfo?.estimatedBytes || estimatedRenderBytes)}</b><span>{renderInfo?.verification?.fileBytes ? 'Actual master size' : 'Estimated before render'}</span></div><div><small>TIME</small><b>{formatTime(renderElapsed, true)}</b><span>{renderEta ? `เหลือประมาณ ${formatTime(renderEta)}` : renderComplete || renderFailed ? 'เสร็จสิ้น' : 'กำลังคำนวณ'}</span></div></div>
        ...
        {renderInfo?.verification ? <div className={`verification-report ${renderInfo.verification.passed ? 'passed' : 'failed'}`}><div className="verification-title">{renderInfo.verification.passed ? <ShieldCheck size={18} /> : <TriangleAlert size={18} />}<span><b>{renderInfo.verification.passed ? 'POST-RENDER VERIFICATION PASSED' : 'POST-RENDER VERIFICATION FAILED'}</b><small>เปิดอ่านไฟล์ที่สร้างเสร็จและตรวจสเปกจริงแล้ว</small></span></div><div className="verification-values"><span><small>CODEC</small><b>{renderInfo.verification.codec || '-'}</b></span><span><small>DURATION</small><b>{formatTime(renderInfo.verification.duration, true)}</b></span><span><small>PEAK</small><b>{Number.isFinite(renderInfo.verification.peakDb) ? `${renderInfo.verification.peakDb.toFixed(2)} dBFS` : '-∞ dBFS'}</b></span><span><small>SAMPLES</small><b>{(renderInfo.verification.sampleCount || 0).toLocaleString()}</b></span></div></div> : renderInfo?.phase === 'error' ? <div className="render-error"><TriangleAlert size={18} /><span><b>RENDER ENGINE ERROR</b><small>{String(renderInfo.error || 'FFmpeg stopped before the master was completed').slice(-360)}</small></span></div> : <div className="render-integrity"><ShieldCheck size={17} /><span><b>QUALITY LOCK ENABLED</b><small>Double-precision mix · High-quality resampling · No lossy codec</small></span></div>}
      </div>
      <footer><span>FFmpeg Lossless Engine · {renderInfo?.freeBytes ? `${formatBytes(renderInfo.freeBytes)} free before render` : api ? 'Checking destination capacity' : 'Web Preview simulation'}</span><div>{renderInfo?.filePath && (renderComplete || renderFailed) && api?.revealFile && <button className="reveal-render" onClick={() => api.revealFile(renderInfo.filePath)}><FolderOpen size={14} /> แสดงไฟล์</button>}<button onClick={cancelRender}>{renderComplete || renderFailed ? <Check size={14} /> : <X size={14} />} {renderComplete || renderFailed ? 'ปิดรายงาน' : 'ยกเลิก Render'}</button></div></footer>
    </section></div>}
  </div>;
}
